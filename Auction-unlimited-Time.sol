...
contract MyERC721 is ERC721, ERC721URIStorage{
    ...
    function startAuction(uint256 _tokenId, uint256 _startingPrice) public {
        require(ownerOf(_tokenId) == msg.sender, "You are not the owner of the token");
        require(_startingPrice > 0, "Price should be gretaer than zero");
        auctions[_tokenId] = Auction(msg.sender, _tokenId, _startingPrice, block.timestamp);
    }

   function placeBid(uint256 _tokenId) public payable {
        require(msg.sender != auctions[_tokenId].seller, "Seller can not place bid");
        require(msg.value > auctions[_tokenId].startingPrice, "Bid value must be greater than starting price");
    ...
    function acceptBid(uint256 _tokenId, uint256 _bidder) public {
        ...
        _transfer(auctions[_tokenId].seller, selectBidder, _tokenId);
        payable(auctions[_tokenId].seller).transfer(selectAmount);
